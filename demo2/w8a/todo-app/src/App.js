...
function TodoItem(props){
  return <p className="card" onClick={} >{props.content}</p>
}
class TodoList extends React.Component {
  ...
  addTask(e){
    ...
    todoList.push(
      { 
        "id": this.currentId, "completed": true, "priority": 1, "content": ""})
        this.currentId++
  ...
  render () {
    return (
      <>
        ...
        .map((v) => <TodoItem id={v.id} removeTask={(id)=> this.removeTask} key = {v.id} content = {v.content}
        priority = {v.prioriry}
